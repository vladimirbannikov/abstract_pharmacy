...
build-impl-%: build_%/Makefile
	@cmake --build build_$* -j $(NPROCS) --target code_architecture
...
service-impl-start-%: build-impl-%
	@cd ./build_$* && $(MAKE) start-code_architecture
...
install-debug: build-debug
	@cd build_debug && \
		cmake --install . -v --component code_architecture
...
--debug-start-in-docker: install
	@sed -i 's/config_vars.yaml/config_vars.docker.yaml/g' /home/user/.local/etc/code_architecture/static_config.yaml
	@psql 'postgresql://user:password@service-postgres:5432/code_architecture_db-1' -f ./postgresql/data/initial_data.sql
	@/home/user/.local/bin/code_architecture \
		--config /home/user/.local/etc/code_architecture/static_config.yaml
...
docker-impl-%:
	docker-compose run --rm code_architecture-service make $*

# Build and runs service in docker environment
docker-start-service-debug:
	@docker-compose run -p 8080:8080 --rm code_architecture-service make -- --debug-start-in-docker-debug

# Build and runs service in docker environment
docker-start-service:
	@docker-compose run -p 8080:8080 --rm code_architecture-service make -- --debug-start-in-docker
